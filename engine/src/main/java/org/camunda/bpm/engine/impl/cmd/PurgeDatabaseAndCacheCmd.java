    ...
    for (String tableName : tablesNames) {
      ...
      if (!TABLENAMES_EXCLUDED_FROM_DB_CLEAN_CHECK.contains(tableNameWithoutPrefix)) {
        ...
        if (!objects.isEmpty()) {
          ...
          //get corresponding entity classes for table which contains data
          List<Class<? extends DbEntity>> entities = commandContext.getTableDataManager()
            .getEntities(tableName, !databaseTablePrefix.isEmpty());

          //set up for each entity type for that table a delete bulk operation
          for (Class<? extends DbEntity> entity : entities) {
